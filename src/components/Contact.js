...
export const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.code === 200) {
      setStatus({ success: true, message: 'Mensaje mandado correctamente'});
    } else {
      setStatus({ success: false, message: 'Algo ha salido mal, intentalo de nuevo más tarde.'});
    }
  };
              ...
              {({ isVisible }) =>
                ...
                <form onSubmit={handleSubmit}>
                  <Row>
                    ...
                    <Col size={12} sm={6} className="px-1">
                      <input type="phone" value={formDetails.phone} placeholder="Teléfono" onChange={(e) => onFormUpdate('phone', e.target.value)}/>
                    </Col>
